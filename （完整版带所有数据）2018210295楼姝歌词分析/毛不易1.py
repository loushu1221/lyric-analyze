# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 16:24:57 2019
...
#定义一个函数：按照歌曲id，提取歌词内容，但是一个一个输ID是不现实的，得要用下一个函数通过歌手ID得到歌曲id
def download_by_music_id(music_id):
    ...
    json_obj = r.text
    j = json.loads(json_obj)#将已编码的 JSON 字符串解码为 Python 对象
    try:
        lrc = j['lrc']['lyric']
        pat = re.compile(r'\[.*\]')
        lrc = re.sub(pat, "",lrc)
...
#再定义一个函数：按照歌手id，发起请求，解析网页，提取歌曲id
